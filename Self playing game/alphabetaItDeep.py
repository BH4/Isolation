...
def neighborFind(node,numFilled):#find the neighbor whose score is max
    kids=children(node)
    scores=[]
    for child in kids:
        c=getChild(node,child)
        myScore=bfsScore(c[0],c[node[3]],numFilled)
        bScore=bfsScore(c[0],child[1],numFilled)
        s=myScore+bScore
        scores.append(s)
...
def bfsScore(board,pos,numFilled):
    board=deepcopy(board)

    #den=int(numFilled/5)+2
...
def play(board,pid):
    ...
    if currDepth-1<7:
        ans=neighborFind(node,numFilled)
        print 'ID cutoff'
    else:

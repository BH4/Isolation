...
def evaluation(board,pos):#heristic to calculate utility of pid's position
    #return len(neighbors(board,pos))
...
def minimax(node,depth,isMe,pid):#node=(board,Ppos,Epos)
    #board=deepcopy(board)
    board=node[0]
    Ppos=node[1]
    Epos=node[2]
    ...
    if isMe:
        ...
        for c in kids:
            ...
            val=minimax((board,Epos,c[0]),depth-1,False,pid)
        ...
        for c in kids:
            ...
            val=minimax((board,Epos,c[0]),depth-1,True,pid)
...
def minimaxW(board,Ppos,Epos,depth,pid):
...
def play(board,pid):
    ...
    ans=minimaxW(board,player,enemy,2,pid)
    return [ans[0],ans[1]]

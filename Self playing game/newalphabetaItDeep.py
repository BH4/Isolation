...
def heuristic(node,maxPlayer,numFilled):
    ...
    myNeigh=neighbors(node[0],node[myId])
    eNeigh=neighbors(node[0],node[oId])
    myMoves=0
    eMoves=0
    for n in myNeigh:
        myMoves+=len(neighbors(node[0],n))
    for n in eNeigh:
        eMoves+=len(neighbors(node[0],n))

    #print time()-t
    
    return (myMoves-eMoves)#*numFilled
"""
def heuristic(node,maxPlayer,numFilled,depth):
    #t=time()
    ...
    myS=dfsScore(node[0],node[myId],[],depth)
    eS=dfsScore(node[0],node[oId],[],depth)


    #print time()-t
    
    return (myS-eS)#*numFilled


def dfsScore(board,pos,seen,depth):
    if pos in seen or board[pos[0]][pos[1]]==-1 or depth==0:
        return 0
    ...
    for m in moves:
        test=(pos[0]+m[0],pos[1]+m[1])
        if test[0]>=0 and test[1]>=0 and test[0]<7 and test[1]<7:
            s+=dfsScore(board,test,seen+[pos],depth-1)+1
    return s
...
def alphabeta(node,depth,alpha,beta,maxPlayer,numFilled):
    ...
    if depth<=0 and len(neighbors(node[0],node[node[3]]))>1:
        #print 'depth 0'
        return heuristic(node,maxPlayer,numFilled,3)
    #if depth<=0 and len(neighbors(node[0],node[node[3]]))<=1:
    #    print 'quintesence!'
...
def minimax(node,depth,numFilled):#always the first call
    ...
    bestChild=None
    kids=children(node)
    kids=neighborSort(node,True,numFilled)
    #print kids
...
def neighborSort(node,maxPlayer,numFilled):
    kids=children(node)
    
    scores=[]
    for child in kids:
        c=getChild(node,child)
        scores.append(heuristic(c,not maxPlayer, numFilled+1,3))

    ziped=zip(scores,kids)
    ziped.sort()
    ziped.reverse()
    scores,kids=zip(*ziped)
    #print scores
...
def play(board,pid):
    ...
    if currDepth-1<7:
        ...
        print 'ID depth='+str(currDepth-1)

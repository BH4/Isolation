...
def spotScore(board,pos,currPlayer):
    n=[]
    ...
    for m in moves:
        test=(pos[0]+m[0],pos[1]+m[1])
        l=legalBlock(board,test,currPlayer)
        lmove.append(l)
        if l:
            recentG+=1
        elif recentG>0:
            groups.append(recentG)
            recentG=0

    if recentG>0:
        groups.append(recentG)
    
    if len(groups)>1 and lmove[0] and lmove[-1]:
        #print board
        #print lmove
        #print groups
        #print '------------------------------------------'
    ...
    return sum([x*x for x in groups])
...
def heuristic(node,maxPlayer,numFilled):
    ...
    myS=spotScore(node[0],node[myId],myId)
    eS=spotScore(node[0],node[oId],oId)

    #print time()-t
    
    return (myS-3*eS)#*numFilled
...
def alphabeta(node,depth,alpha,beta,maxPlayer,numFilled):
    ...
    kidNodes,kids=kidSort(node,True)
    
    if maxPlayer:
        #print 'max player'
        val=-10**7
        for child in kidNodes:
            #c=getChild(node,child)#gets child node

            ab=alphabeta(child,depth-1,alpha,beta,False,numFilled+1)
            if ab==None:
                return None
            nodeValueDict[str(child)]=ab
        ...
        for child in kidNodes:
            ...
            #print c[0]
            #raw_input()
            ab=alphabeta(child,depth-1,alpha,beta,True,numFilled+1)
            if ab==None:
                return None
            nodeValueDict[str(child)]=ab
...
def minimax(node,depth,numFilled):#always the first call
    ...
    val=-10**7
    bestChild=None
    kidNodes,kids=kidSort(node,True)#children(node)
    
    for i,child in enumerate(kidNodes):
...
def kidSort(node,maxPlayer):
    ...
    for child in kids:
        ...
        kidNodes.append(c)
        key=str(c)
        if key in nodeValueDict:
            scores.append(nodeValueDict[key])
        else:
            scores.append(0)

    ziped=zip(scores,kidNodes,kids)
    ziped.sort()
    if not maxPlayer:
        ziped.reverse()
    scores,kidNodes,kids=zip(*ziped)

    return (kidNodes,kids)
...
def closeNeighborFind(node):#find the neighbor whose score is max
    kids=children(node)
    scores=[]
    for child in kids:
        c=getChild(node,child)
        s=heuristic(c,False,0)
